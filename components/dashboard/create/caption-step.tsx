...
export function CaptionStep({ formData, setFormData }: any) {
  const selectedStyle = captionStyles.find(s => s.name === formData.captionStyle) || captionStyles[0]
          ...
          {captionStyles.map((style) => (
            ...
            >
              ...
              <div className={cn(
                ...
                formData.captionStyle === style.name ? "border-purple-600 bg-purple-600" : "border-white/10"
              )}>
                 {formData.captionStyle === style.name && <div className="h-2 w-2 bg-white rounded-full" />}
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="space-y-6">
           ...
           <div className="aspect-video rounded-3xl overflow-hidden relative border border-white/10 bg-zinc-900 group">
              <img 
                ...
                className="absolute inset-0 h-full w-full object-cover brightness-[0.4] blur-[2px] transition-transform duration-1000 group-hover:scale-105"
                alt="Preview"
              />
